...
function drawKpi() {
  ...
  $('#ovKpi').innerHTML = [
    ...
    kpi('氣候基期', S.meta.base[0] + '–' + S.meta.base[1], (S.meta.base[1] - S.meta.base[0] + 1) + ' 年' + ((S.meta.base[1] - S.meta.base[0] + 1) >= 30 ? ' · 符合 WMO 標準' : ' · 短於 WMO 30 年標準，趨勢僅供參考'), 'acc'),
...
function methodHtml() {
  ...
  return [
    ...
    '<h3>3. 氣候基期與距平</h3>',
    '<p>採 <b>' + (m.base ? m.base[0] + '–' + m.base[1] : '1991–2020') + '</b> 之基期' + (m.base && (m.base[1] - m.base[0] + 1) < 30 ? '（' + (m.base[1] - m.base[0] + 1) + ' 年，短於 WMO 30 年標準；目前資料檔為水試所衛星海溫圖數位化之短記錄，長期趨勢與熱浪統計僅供參考）' : '（30 年，WMO 標準）') + '。逐格、逐日序氣候值以 <b>±5 日視窗</b>（共 11 日）跨基期各年取平均，再沿日序做 <b>31 日環狀移動平均</b>平滑，即 Hobday et al. (2016) 之標準流程。距平 = 觀測 − 該日序氣候值。</p>',
    '<h3>4. 海洋熱浪判定</h3>',
